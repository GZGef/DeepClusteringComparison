...
def main():
    ...
    df_normalized, df_original, feature_names = load_and_preprocess_data(
        url="https://storage.yandexcloud.net/google-colab-bucket/Datasets/Mall_Customers.csv",
    ...
    # Силуэтный анализ
    silhouette_scores = find_optimal_k_methods.silhouette_analysis(
        df_normalized, max_k=15
    )
    ...
    # Создание DEC модели
    dec_model = DEC(
        autoencoder=autoencoder, n_clusters=n_clusters, latent_dim=2, alpha=1.0
    ).to(device)
    ...
    print("\n=== 10. АНАЛИЗ СТАТИСТИКИ ПО КЛАСТЕРАМ ===")
    analyze_cluster_statistics(
        df_original.values, kmeans_labels, feature_names, "K-Means"
    )
    analyze_cluster_statistics(df_original.values, dec_labels, feature_names, "DEC")
